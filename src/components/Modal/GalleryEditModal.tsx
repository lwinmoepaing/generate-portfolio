...
interface ModalInterface {
  onUpdateGallery: (item: GalleryDoc) => void | any;
  onCloseModal: () => void | any;
  gallery: GalleryDoc;
}

const GalleryEditModal: React.FC<ModalInterface> = ({
  onCloseModal,
  onUpdateGallery,
  gallery,
}) => {
  ...
  const [phoneText, setPhoneText] = useState<string>(gallery.phone || "");
  const [url, setUrl] = useState<string>(gallery.url || "");
  const [actionType, setActionType] = useState<
    "url" | "alert" | "tel" | "disabled"
  >(gallery.action_type);

  const [isTouched, setIsTouched] = useState<boolean>(false);
  const [radio1, radio2, radio3, radio4] = [
    nanoid(),
    nanoid(),
    nanoid(),
    nanoid(),
              ...
              <div className="flex items-start my-4">
                ...
                <div className="form-check form-check-inline">
                  <input
                    ...
                    id={radio4}
                    value="disabled"
                    defaultChecked={actionType === "disabled"}
                    onChange={handlerActionType}
                  />
                  <label
                    className="form-check-label inline-block text-gray-800"
